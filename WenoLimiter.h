...
namespace limiter {
  ...
  template <> struct WenoLimiter<5> {
    ...
    YAKL_INLINE WenoLimiter(real cutoff_in,
                            real idl_L_in ,
                            real idl_C_in ,
                            real idl_R_in ,
                            real idl_H_in ) {
    ...
    YAKL_INLINE static void compute_limited_coefs( SArray<real,1,5> const &s , SArray<real,1,5> &coefs_H , Params const &params ) {
      ...
      w_L = params.idl_L / (w_L*w_L + 1.e-20);
      w_C = params.idl_C / (w_C*w_C + 1.e-20);
      w_R = params.idl_R / (w_R*w_R + 1.e-20);
      w_H = params.idl_H / (w_H*w_H + 1.e-20);
      convexify( w_L , w_C , w_R , w_H );
      if (w_L <= params.cutoff) w_L = 0;
      if (w_C <= params.cutoff) w_C = 0;
      if (w_R <= params.cutoff) w_R = 0;
      convexify( w_L , w_C , w_R , w_H );
